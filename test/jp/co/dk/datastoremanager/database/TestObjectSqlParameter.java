...
public class TestObjectSqlParameter extends TestDataStoreManagerFoundation{
	...
	public void constractor() throws ParseException {
		...
		// コンストラクタに文字列オブジェクトを設定した場合、正常に値が設定できること。
		try {
			ObjectSqlParameter parameter = new ObjectSqlParameter(null);
		} catch (DataStoreManagerException e) {
		...
		// コンストラクタに文字列オブジェクトを設定した場合、正常に値が設定できること。
		try {
			ObjectSqlParameter parameter = new ObjectSqlParameter("");
		} catch (DataStoreManagerException e) {
			assertEquals(e.getMessageObj(), SQL_PARAMETER_IS_NOT_SET);
		}
	}
